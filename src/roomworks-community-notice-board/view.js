/**
 * Progressively enhances the server-rendered notice board: intercepts the
 * filter form and pagination links to fetch from the REST endpoint instead
 * of doing a full page reload. Without this script (or if the fetch fails)
 * the form/links still work as plain GET requests, since render.php
 * performs the same query server-side from $_GET. Near-identical port of
 * roomworks-business-networking/view.js - see that file for the general
 * approach; only the filter fields and card fields differ (job type/
 * category/budget/closing date in place of service/logo).
 *
...
document.addEventListener(
	'DOMContentLoaded',
	function () {
		const roots = document.querySelectorAll( '[data-rbn-notice-board]' );
			...
			function ( root ) {
				...
				function currentFilters() {
					...
					return {
						...
						urgency: data.get( 'rbn_urgency' ) || '',
						location: data.get( 'rbn_location' ) || '',
					};
				}

				function hasActiveFilters( filters ) {
					return Boolean( filters.search || filters.category || filters.urgency || filters.location );
				}
				...
				function createCard( item ) {
					...
					card.className = 'rbn-job-card rbn-card';

					const body     = document.createElement( 'div' );
					body.className = 'rbn-job-card__body';
					card.appendChild( body );

					if ( item.category ) {
						const category       = document.createElement( 'p' );
						category.className   = 'rbn-job-card__category';
						category.textContent = item.category;
						body.appendChild( category );
					}

					const title           = document.createElement( 'h3' );
					title.className       = 'rbn-job-card__title';
					...
					if ( item.urgency ) {
						...
						urgency.className   = 'rbn-job-card__urgency';
					...
					if ( item.excerpt ) {
						...
						excerpt.className   = 'rbn-job-card__excerpt';
					...
					if ( location ) {
						...
						locationEl.className = 'rbn-job-card__location';
					...
					if ( item.budget ) {
						...
						budget.className   = 'rbn-job-card__budget';
					...
					if ( item.closing_date ) {
						...
						closing.className   = 'rbn-job-card__closing';
						closing.textContent = ( i18n.closes || 'Closes %s' ).replace( '%s', item.closing_date );
						body.appendChild( closing );
					}

					const linkP      = document.createElement( 'p' );
					linkP.className  = 'rbn-job-card__link';
					const link       = document.createElement( 'a' );
					link.href        = item.permalink;
					link.textContent = i18n.viewDetails || 'View Details';
				...
				function renderResults( items ) {
					...
					if ( ! items.length ) {
						...
						empty.textContent = i18n.noResults || 'No requests found.';
				...
				function renderPagination( data ) {
					...
					nav.setAttribute( 'aria-label', 'Notice board pagination' );
				...
				function fetchResults( page ) {
					...
					statusEl.textContent = i18n.loading || 'Loading requests…';
				...
				if ( filterActions ) {
					filterActions.addEventListener(
						...
						function ( event ) {
							...
							[ 'rbn_search', 'rbn_category', 'rbn_urgency', 'rbn_location' ].forEach(
								function ( name ) {
